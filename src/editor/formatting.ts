...
const unwrapLink = (editor: Editor) => {
  Transforms.unwrapNodes(editor, {
    match: (n) =>
      ...
      Element.isElement(n) &&
      (
        n.type === ElementType.ExternalLink || 
...
const wrapLink = (editor: Editor, link: ExternalLink | NoteLink | PubLink) => {
  const { selection } = editor;
  if (!selection) {
    return;
  }
  ...
  if (
    ...
    isElementActive(editor, ElementType.PubLink)
  ) {
    unwrapLink(editor);
  }
