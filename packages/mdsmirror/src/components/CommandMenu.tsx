...
import attachFiles, { Attach } from "../core/commands/attachFiles";
...
class CommandMenu<T extends MenuItem = MenuItem> extends React.Component<Props<T>, State> {
  ...
  handleLinkInputKeydown = (event: React.KeyboardEvent<HTMLInputElement>) => {
    ...
    if (event.key === "Enter") {
      ...
      const matches = this.state.insertEmbed.matcher(href);
  ...
  triggerLinkInput = (item: EmbedDescriptor) => {
    this.setState({ insertEmbed: item });
  };
  ...
  handleFilePicked = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!uploadFile) {
      throw new Error("no uploadFile callback provided");
    }
  ...
  get filtered() {
    ...
    for (const embed of embeds) {
      if (embed.title && embed.icon) {
        embedItems.push({ ...embed, name: "embed" });
      }
    }
